...
Among feasible itineraries, the system should prefer those that best match the user. We treat these as \emph{soft} objectives that guide selection but can be traded off.

\paragraph{O1: Preference alignment (personalization).}
An itinerary should maximize alignment with the user's preference representation $\mathbf{p}_u$, expressed through a per-POI relevance score $r(u, v, c)$ learned from data.

\paragraph{O2: Context alignment.}
The itinerary should be suitable for the observed spatial/temporal context, captured by context-conditioned scoring---a different relevance for the same POI under different $c$ \citep{adomavicius2005tois, adomavicius2022cars}.

\paragraph{Further quality objectives (scoped as future work).}
Geographic coherence (minimizing travel effort) and category diversity are also desirable and can be added as travel-cost penalties and diversity rewards in the decoding objective; opening-hour/time-window feasibility can likewise be enforced when such metadata is available. Because the realized system enforces only the budget and uniqueness constraints (Section~\ref{sec:sol-decoding}), these terms are treated as extensions rather than part of the evaluated method (Chapter~\ref{ch:conclusion}).

Two points follow. First, itinerary generation is inherently \textbf{multi-objective}: the best itinerary is not simply the set of top-ranked individual POIs. Second, feasibility is a generation-time concern rather than a post-processing detail, which requires algorithms that reason over \emph{sequences} rather than independent POIs.
...
Sequential POI recommenders are trained for next-POI prediction but, when deployed with greedy decoding, can produce globally poor routes despite locally plausible steps: myopic choices that exhaust the budget early, zigzag routes that harm geographic coherence, and budget violations when feasibility is not enforced during generation. We call this the \emph{greedy trap} (illustrated in Chapter~\ref{ch:introduction}, Figure~\ref{fig:greedy-trap}); producing coherent sequences requires inference-time strategies that look beyond the next step \citep{wu2025tlmr}, which motivates \textbf{RQ3}.
