...
This chapter provides a comprehensive review of the state of the art organized as a \emph{field taxonomy} rather than a problem-specific literature review. The taxonomy, illustrated in Figure~\ref{fig:taxonomy}, structures existing work along three complementary facets: (i)~the user-facing objective (from non-personalized to personalized and context-aware recommendation), (ii)~the computational approach employed (classical methods, deep learning, and emerging paradigms), and (iii)~the supporting data sources and evaluation practice. Each top-level branch is expanded in a dedicated figure as its sections are reached (Figures~\ref{fig:tax-user}, \ref{fig:tax-comp}, and~\ref{fig:tax-sys}). This organization extends the taxonomy proposed by Halder~\cite{halder2022thesis}, which covered the landscape through 2022, by incorporating major developments from 2022 to 2026---notably graph neural networks with contrastive learning, self-supervised pretraining, large language model-based travel planning, and multimodal context integration.
...
\textbf{PersTour.} The PersTour system of Lim et al.~\cite{lim2015photo} personalizes tour planning by reading each user's interests from photo trails: the longer a visitor lingers at a venue, the stronger their inferred interest in it. These per-user interest weights replace the generic popularity score inside an orienteering formulation, so that the recommended route is tailored to the individual while still respecting the trip's time budget and start/end points.

\textbf{PersQ.} PersQ~\cite{lim2017persq} adds a third ingredient---time spent queuing at attractions---and balances interest and popularity against expected waiting time. Its main novelty is algorithmic: it frames itinerary construction as a single-player game and explores it with Monte Carlo Tree Search (MCTS), the first transfer of this game-playing technique into tour recommendation.

\textbf{EffiTourRec.} EffiTourRec~\cite{halder2022effitour} refines the PersQ line in two respects. It corrects the interest signal so that a longer visit \emph{raises} rather than lowers the inferred interest in a POI, and it prunes the MCTS search so that far fewer states are explored without sacrificing recommendation quality. On Disney theme-park data the authors report large reductions in both search effort and runtime relative to earlier MCTS planners.
...
In the session-based recommendation setting, Hidasi et al.~\cite{hidasi2016gru4rec} introduced GRU4Rec, applying GRU networks to model user sessions (short interaction sequences) without requiring long-term user profiles. This approach is relevant to tourism, where a visitor's city exploration can be viewed as a session with limited history. Baral et al.~\cite{baral2018capsrnn} modelled POI visit sequences with an RNN conditioned on each visitor's interests and situational context, although the effect of inter-POI distance was left unmodelled.

The MSTP-LSTM model by Hu et al.~\cite{hu2018mstp} incorporated multi-source data including travelogues and check-in information to capture user interests across heterogeneous inputs. Debnath et al.~\cite{debnath2019pttr} conditioned a route recommender jointly on travel time and user preference, finding that accounting for both dwell and transit durations improves the quality of the planned tour.
...
Within POI prediction, STrans~\cite{lian2020strans} used self-attention to model spatial and temporal dependencies between check-ins jointly, reporting gains over recurrent baselines on the location-prediction task. Halder et al.~\cite{halder2022tlr} applied multi-head self-attention to POI sequences with spatio-temporal features in their TLR model, and---through a multi-task head (TLR-M)---predicted POI rankings and queuing time together rather than as separate problems, an early example of jointly learning the two. Their description-based variants (TLR\_UI, TLR-M\_UI) additionally inject an interest signal derived from POI text, which lets the model score venues it has never seen and so eases the new-POI cold-start problem.
...
\textbf{Early GCN-based approaches.} The first graph models borrowed convolutional architectures from traffic forecasting: T-GCN~\cite{zhao2019tgcn} couples a GCN with a GRU, and STGCN~\cite{yu2018stgcn} applies spectral graph convolutions over spatial-temporal signals. Their weakness for tourism is that they were designed for regular road or grid networks and do not naturally represent the irregular topology of a POI co-visitation graph.
