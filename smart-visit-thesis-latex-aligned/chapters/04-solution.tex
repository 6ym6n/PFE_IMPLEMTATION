...
\input{figures/chart-nextpoi-tier}
...
literature's own benchmark. Figure~\ref{fig:chart-scale} makes this scale gap explicit.

\input{figures/chart-scale}
...
\input{figures/chart-validation}
...
Markov-family baselines (\texttt{Markov} and \texttt{MarkovPath}) are strongest, and the light
from-scratch pointer (0.31--0.49) reaches the scale and beats \texttt{Random} comfortably but
trails \texttt{Markov} on every city---the same supervision-density lesson seen on NYC.
...
Markov (greedy) & 0.504 & \textbf{0.421} & \textbf{0.587} & 0.449 & 0.333 \\
MarkovPath (beam) & \textbf{0.528} & 0.398 & 0.543 & \textbf{0.452} & \textbf{0.346} \\
...
\input{figures/chart-flickr-pub}
